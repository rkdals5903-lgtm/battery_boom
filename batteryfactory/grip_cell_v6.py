#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""R_4 battery-cell sorting/stacking controller (v6).
...
# Calibrated placement pose requested for the relocated robot.
NEW_CASE_FINAL_ROOT_TARGET = np.array([1.60817, 6.11489, 0.99122], dtype=float)
...
FACTORY_FLOOR_CELL_ROOT_Z_M = 0.0
...
INSPECTION_EXTRA_Y_OFFSET_M = 0.225
...
def rotate_joint1_for_reject(world, robot, controller, follower, home_joint_pose):
    ...
    max_step = 0.35 * transfer.base.PHYSICS_DT
    print(
...
def _run_single_cell_impl(cell_index=1, stack_index=1):
    ...
    inspection_cell_position, _ = cell_visual.get_world_pose()
    ...
    if inspection_ok:
        ...
        approach_root = final_root + np.array(
            [0.0, 0.0, NEW_CASE_VERTICAL_APPROACH_M]
        )
        approach_link6 = approach_root - follower.offset
        final_link6 = final_root - follower.offset
        print(
            "[NEW_CASE ROOT TARGET] source="
            f"{np.round(initial_root_position, 5)}, approach={np.round(approach_root, 5)}, "
            f"final={np.round(final_root, 5)}"
        )
        ...
        x_aligned_root = np.asarray(retreat_cell_root, dtype=float).copy()
        x_aligned_root[0] = approach_root[0]
        x_aligned_root[2] = approach_root[2]
        x_aligned_link6 = x_aligned_root - follower.offset
        print(
            "[NEW_CASE AXIS PATH] retreat="
            f"{np.round(retreat_cell_root, 5)} -> X-aligned="
            f"{np.round(x_aligned_root, 5)} -> XY-aligned={np.round(approach_root, 5)}"
        )
        runner.move(
            x_aligned_link6,
        ...
        x_actual, _ = cell_visual.get_world_pose()
        x_error = np.asarray(x_aligned_root) - np.asarray(x_actual)
        print(
            "[NEW_CASE X VERIFY] actual="
            f"{np.round(x_actual, 5)}, error={np.round(x_error, 5)}"
        )
        ...
        approach_actual, _ = cell_visual.get_world_pose()
        approach_error = np.asarray(approach_root) - np.asarray(approach_actual)
        print(
            "[NEW_CASE APPROACH VERIFY] actual="
            f"{np.round(approach_actual, 5)}, error={np.round(approach_error, 5)}"
        )
        ...
        final_actual, _ = cell_visual.get_world_pose()
        final_error = np.asarray(final_root) - np.asarray(final_actual)
        print(
            "[NEW_CASE PLACE VERIFY] actual="
            f"{np.round(final_actual, 5)}, error={np.round(final_error, 5)}"
        )
        ...
        # guarded placement error is acceptable, author the exact calibrated
        # root pose before opening the fingers so the released cell coordinate
        # is not offset by the remaining RMPFlow residual.
        cell_visual.set_world_pose(final_root, initial_orientation)
        print(f"[NEW_CASE FINAL SNAP] root={np.round(final_root, 5)}")
        set_rg2_carry_collision_filter(stage, False)
        command_gripper(
            world, robot, controller, gripper_dof_indices,
        ...
        drop_start = np.asarray(drop_start, dtype=float)
        drop_end = drop_start.copy()
        drop_end[2] = FACTORY_FLOOR_CELL_ROOT_Z_M
        for alpha in np.linspace(0.0, 1.0, 120):
            cell_visual.set_world_pose(
                (1.0 - alpha) * drop_start + alpha * drop_end,
                drop_orientation,
            )
            world.step(render=True)
        print(f"[REJECT DROP] cell root={np.round(drop_end, 5)}")
    ...
    print(f"[COMPLETE] cell_1 inspection result={inspection_ok} -> {result_label}")
...
class R4StateMachine:
    """Three-node process coordinator for the pull-out robot."""

    def __init__(self):
        self.cell_count = 1
        self.stack_count = 1
        self.max_cells = 4
        self.camera_service = os.environ.get("R4_CAMERA_SERVICE", "/gripper_camera/inspect")
        self.voltage_service = os.environ.get("R4_VOLTAGE_SERVICE", "/voltage_inspection/check")
        self.lid_trigger = os.environ.get("R4_LID_TRIGGER", "/suction_robot/lid_open_done")
        self.close_trigger = os.environ.get("R4_CLOSE_TRIGGER", "/suction_robot/close_lid")
        self.auto_start = os.environ.get("R4_AUTO_START", "0") == "1"

    def trigger(self, service):
        """Blocking std_srvs/Trigger call; service names are configurable."""
        print(f"[R4 WAIT] {service}", flush=True)
        command = (
            "source /opt/ros/humble/setup.bash && "
        ...
        result = subprocess.run(
            ["/bin/bash", "-lc", command],
            text=True, capture_output=True, env=ros_environment,
        )
        if result.returncode != 0:
            raise RuntimeError(f"Trigger failed: {service}: {result.stderr.strip()}")
        output = result.stdout.lower()
        return "success: true" in output or "success=true" in output

    def camera_ok(self):
        return self.trigger(self.camera_service)

    def voltage_ok(self):
        # Voltage node owns the measurement and threshold (value > 500).
        return self.trigger(self.voltage_service)

    def run(self):
        print("[R4 INIT] cell_count=1, stack_count=1")
        while True:
            if not self.auto_start and not self.trigger(self.lid_trigger):
                continue
            print("[R4 START] lid-open trigger received" if not self.auto_start else "[R4 START] auto-start enabled")
            while self.cell_count <= self.max_cells:
                print(f"[R4 PICK] old cell_count={self.cell_count}")
                if not self.camera_ok():
                    print("[R4 REJECT] camera=False -> disposal zone")
                    self.cell_count += 1
                    continue
                if not self.voltage_ok():
                    print("[R4 REJECT] voltage<=500 -> disposal zone")
                    self.cell_count += 1
                    continue
                print(f"[R4 PLACE] new stack_count={self.stack_count}")
                run_single_cell(self.cell_count, self.stack_count)
                self.cell_count += 1
                if self.stack_count < self.max_cells:
                    self.stack_count += 1
                else:
                    self.stack_count = 1
                    self.trigger(self.close_trigger)
                    break
            self.cell_count = 1
            self.stack_count = 1


def run_single_cell(cell_index=1, stack_index=1):
    global CELL_PATH, CELL_JOINT_PATH
    CELL_PATH = f"/World/good_battery/cell_{int(cell_index)}"
    CELL_JOINT_PATH = f"/World/good_battery/AssemblyJoints/cell_{int(cell_index)}_to_casebase"
    return _run_single_cell_impl(cell_index, stack_index)
