...
SAFE_BATTERY_ASSET_PATH = THIS_DIR / "Collected_factory_clean" / "small_cell_battery_staged_meters_assembly_safe.usd"
...
def replace_battery_with_safe_asset(stage):
    """Replace only the battery instance in the factory scene with the safe USD asset."""
    if not SAFE_BATTERY_ASSET_PATH.is_file():
        raise FileNotFoundError(SAFE_BATTERY_ASSET_PATH)
    old_battery = stage.GetPrimAtPath(BATTERY_ROOT)
    if old_battery.IsValid():
        stage.RemovePrim(BATTERY_ROOT)
    battery = stage.DefinePrim(BATTERY_ROOT, "Xform")
    battery.GetReferences().AddReference(str(SAFE_BATTERY_ASSET_PATH.resolve()))
    if not stage.GetPrimAtPath(CASECOVER).IsValid():
        raise RuntimeError(
            "safe battery asset reference did not produce the expected hierarchy: "
            f"{CASECOVER}"
        )
    print(f"[SAFE BATTERY] referenced asset: {SAFE_BATTERY_ASSET_PATH}")
...
def main() -> None:
    ...
    replace_battery_with_safe_asset(stage)
