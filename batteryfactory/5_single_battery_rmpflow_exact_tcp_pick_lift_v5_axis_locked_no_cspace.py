...
# -*- coding: utf-8 -*-
"""
Isaac Sim 5.1 Standalone - M0609 RMPFlow link_6 +Z 지면 고정 / no c-space
...
17. 실제 RMPflow 키인 cspace_target_rmp를 생성 YAML에서 완전히 비활성화하고
    set_cspace_target()을 호출하지 않아 특정 초기 관절 자세로 끌어당기지 않는다.
...
isaac_python 5_single_battery_rmpflow_exact_tcp_pick_lift_v5_axis_locked_no_cspace.py
"""
...
URDF_FILE_PATH = _THIS_DIR / "_generated_m0609_v5_axis_locked_no_cspace.urdf"
RMPFLOW_CONFIG_PATH = _THIS_DIR / "_generated_m0609_v5_axis_locked_no_cspace.yaml"
...
# 현재 단계는 궤적/흡착 검증이 목적이므로 배터리 부하를 사실상 제거한다.
# 질량 0은 PhysX가 밀도 기반 질량을 다시 계산할 수 있어 작은 양수값을 사용한다.
...
# 첫 번째 ConveyorTrack의 중심은 USD 기준 약 (1.333, 0.0)이고 길이 방향은 Y축이다.
# 중앙까지 뻗지 않고 로봇에 가까운 벨트 안쪽 가장자리에 배터리 루트를 놓는다.
CONVEYOR_DESTINATION = np.array(
    [1.15000, 0.30000, 0.95435], dtype=float
)
...
# VG10 / RMPFlow 동작 튜닝
# -----------------------------------------------------------------------------
# link_6 원점에서 화면에 보이는 VG10 흡착면까지 link_6 로컬 +Z 방향 거리.
# 0.20 m 설정에서는 가상 Surface Gripper만 접촉하고 VG10 모델은 약 10 cm 위에
# 떠 있었으므로 실제 장착 형상에 맞춰 0.10 m로 보정한다.
VG10_TOOL_LENGTH_M = 0.100

# Surface Gripper가 물체를 찾는 최대 거리
# 멀리서도 붙어 버리는 현상을 막기 위해 흡착 탐색 거리를 8 mm로 제한한다.
...
# 컨베이어 목적지는 로봇 작업반경 바깥쪽에 있으므로 지나치게 높은 이송 자세는
# 컨베이어 위에서 던지듯 해제할 수 있도록 배터리 윗면 기준 20 cm를 확보한다.
TRANSFER_CLEARANCE_M = 0.200
...
# 최종 흡착 접근은 일반 waypoint보다 훨씬 엄격하게 판정한다.
GRASP_POSITION_TOLERANCE_M = 0.003
# Place는 벨트/배터리 접촉 반력 때문에 3 mm 수렴이 어려우므로, 벨트 상공 25 mm
# 이내에서 흡착을 풀고 가벼운 배터리가 중력으로 안착하게 한다.
PLACE_POSITION_TOLERANCE_M = 0.025
# 던지듯 내려놓는 마지막 Place에서만 5도까지 허용한다. 이동/흡착 중 2도 기준은 유지한다.
PLACE_AXIS_TOLERANCE_RAD = math.radians(5.0)
# 접촉 때문에 최종 pose가 timeout되어도 벨트 목표에서 5 cm 이내면 안전하게 해제한다.
PLACE_RELEASE_FALLBACK_DISTANCE_M = 0.050
...
GRIP_WAIT_STEPS = 180
# D6 해제와 배터리의 벨트 안착이 화면에 반영될 시간을 확보한다(약 1초).
...
# M0609 USD의 drive stiffness는 0이 아니어도 약 11 수준으로 저장돼 있어 중력을
# 버티지 못한다. 첫 physics step 전에 아래 안전 position gain을 강제로 적용한다.
...
# J4/J5/J6이 link_6 자세를 자유롭게 보상하도록 c-space 자세 attractor를 사용하지 않는다.
# robot_description의 default_q나 Stage 초기 관절값을 c-space 목표로 전달하지 않는다.
DISABLE_CSPACE_TARGET_RMP = True
...
def prepare_joint_limited_rmpflow_files() -> None:
    ...
    # 원본 YAML의 실제 Lula RMPflow 키는 `cspace_target_rmp`이다. 이전 버전은
    # `c_space_target_rmp`를 새로 만들어 원래의 강한 50/100 정책을 그대로 남겼다.
    # 잘못된 별칭은 제거하고 실제 정책의 attractor와 damping metric을 모두 끈다.
...
def create_vg10_surface_gripper(
    ...
    # 파지/해제 시 생성·삭제되는 D6 관계를 Stage에도 반영한다. Fabric 내부 상태만
    # 바뀌고 USD/Physics 조인트가 남아 해제가 보이지 않는 버전 차이를 방지한다.
    try:
...
def configure_articulation_controller(robot: SingleArticulation) -> object:
    ...
    should_apply_safe_gains = FORCE_SAFE_POSITION_GAINS or (
        AUTO_FIX_ZERO_DRIVE_GAINS and (gains_invalid or efforts_invalid)
    )
    if should_apply_safe_gains:
        reason = (
            "중력 낙하 방지를 위해 강제 적용"
            if FORCE_SAFE_POSITION_GAINS
            else "Joint Drive가 0 또는 무효"
        )
...
class RmpFlowRunner:
    def __init__(
        ...
        # 의도적으로 set_cspace_target()을 호출하지 않는다. J4/J5/J6은 초기값이나
        # robot_description의 default_q가 아니라 EE 위치/자세 정책과 관절 제한에
        # 따라서만 보상한다.
        ...
        print(
            "cspace target                    = "
            + "DISABLED (no set_cspace_target call)"
        )
    ...
    def move_to(
        ...
        axis_tolerance_rad: float = GROUND_FACING_TOLERANCE_RAD,
    ) -> None:
        ...
        for step_index in range(max_steps):
            ...
            if (
                ...
                and axis_error <= float(axis_tolerance_rad)
            ):
                stable_steps += 1
            else:
                stable_steps = 0
    ...
    def move_vertical_to(
        ...
        final_position_tolerance_m: float = POSITION_TOLERANCE_M,
        final_axis_tolerance_rad: float = GROUND_FACING_TOLERANCE_RAD,
    ) -> None:
        ...
        for index in range(1, count + 1):
            ...
            self.move_to(
                ...
                axis_tolerance_rad=(
                    final_axis_tolerance_rad
                    if index == count
                    else GROUND_FACING_TOLERANCE_RAD
                ),
            )
...
def open_gripper(
    ...
    gripper_view: GripperView,
    gripper_path: str,
    released_object_path: Optional[str] = None,
) -> None:
    """흡착 해제를 명령하고 실제 D6 파지 관계가 사라졌는지 검증한다."""
    ...
    for attempt in range(1, 4):
        ...
        try:
            interface_held = list(
                gripper_interface.get_gripped_objects(gripper_path) or []
            )
        ...
        if released_object_path is not None:
            interface_still_holding = any(
                held_path == released_object_path
                or held_path.startswith(released_object_path + "/")
                or released_object_path.startswith(held_path + "/")
                for held_path in interface_held_paths
            )
            view_still_holding = any(
                held_path == released_object_path
                or held_path.startswith(released_object_path + "/")
                or released_object_path.startswith(held_path + "/")
                for held_path in view_held_paths
            )
        else:
        ...
        print(
            f"  status={last_status}, interface_held={interface_held}, "
            f"view_held={last_held}"
        )
        # 실제 close 검증에도 사용한 저수준 인터페이스를 판정 기준으로 삼는다.
        # GripperView는 physics step 한두 프레임 동안 이전 held 경로를 반환할 수 있다.
        if interface_query_ok and not interface_still_holding:
            print(
                "  [OK] Surface Gripper 저수준 인터페이스에서 흡착 해제 확인 "
                f"(view_stale={view_still_holding})"
            )
            return
    ...
    raise RuntimeError(
        "Surface Gripper open 명령 후에도 배터리 파지 관계가 남아 있습니다.\n"
        f"gripper={gripper_path}\n"
        f"target={released_object_path}\n"
        f"status={last_status}, held={last_held}"
    )
...
def build_single_pick_tcp_targets(
    ...
    """선택된 배터리의 Pick부터 컨베이어 Place까지 TCP 목표를 계산한다."""
    root_position, _ = get_prim_world_pose(stage, battery_path)
    bbox_min, bbox_max, dimensions = compute_world_bbox(stage, battery_path)
    ...
    # CONVEYOR_DESTINATION은 배터리 루트의 최종 좌표다. 현재 배터리에서 측정한
    # root->윗면 높이를 그대로 더해 컨베이어 위 흡착면 TCP를 계산한다.
...
def run_pick_and_place_sequence(
    ...
    # 명시적인 지면 방향 orientation과 보정된 VG10 tool offset을 사용해 변환한다.
    ...
    # 1) 현재 EE 위치에서 자세만 바꾸는 것은 새 quaternion과 함께 도달 불가능할 수
    #    있다. 실제 로그에서도 자세는 맞았지만 위치가 39.5 cm 밀리며 timeout 됐다.
    #    따라서 별도 사전 정렬을 하지 않고, 도달 가능한 배터리 안전 상공 pose로
    #    위치와 지면 방향 자세를 함께 수렴시킨다. 이 첫 이동 동안에만 초기 축 오차
    #    hard-stop을 유예하고 도착 판정의 2도 조건은 그대로 유지한다.
    runner.move_to(
        overhead_link6,
    ...
    # 2) 상공 좌표에서 같은 지면 방향 orientation을 능동 유지하며 2초 대기한다.
    runner.hold_ground_locked_pose(
        BATTERY_OVERHEAD_HOLD_SECONDS,
        "VG10 TCP 상공 좌표에서 2초 유지",
    )

    # 3) 집을 배터리만 RMPFlow 장애물에서 제외하고 Z축으로만 하강한다.
    ...
    runner.move_vertical_to(
        ...
        final_position_tolerance_m=GRASP_POSITION_TOLERANCE_M,
    )
    ...
    # 5) 동일한 XY/지면 방향 orientation을 유지하며 계산된 안전 상공까지 들어 올린다.
    runner.move_vertical_to(
        lift_link6,
    ...
    # 6) 배터리를 든 채 안전 높이에서 컨베이어 목적지 상공으로 평행 이동한다.
    runner.move_to(
        transfer_link6,
        f"컨베이어 상공 TCP {np.round(transfer_tcp_target, 5)}로 이송",
        timeout_s=MOVE_TIMEOUT_S,
        # 정밀 Place가 아니라 벨트 기준 약 20 cm 상공에 들어오면 즉시 해제한다.
        position_tolerance_m=0.080,
        axis_tolerance_rad=GROUND_FACING_HARD_STOP_RAD,
    )

    # 7) 정밀 하강은 생략한다. 벨트 상공 이송 목표에 도착한 즉시 흡착을 풀어
    #    약 TRANSFER_CLEARANCE_M 높이에서 배터리를 벨트 위로 떨어뜨린다.
    print(
        "[PLACE] 벨트 상공 도착 - 수직 하강 없이 즉시 Surface Gripper 흡착 해제 "
        f"(drop height≈{TRANSFER_CLEARANCE_M:.3f}m)"
    )
    open_gripper(
        world,
        gripper_interface,
        gripper_view,
        gripper_path,
        released_object_path=battery_path,
    )

    # 8) 놓은 배터리를 새 위치의 RMPFlow 장애물로 다시 등록하고 수직 후퇴한다.
    if REENABLE_PLACED_BATTERY_OBSTACLE:
        runner.set_battery_obstacle_enabled(battery_name, True)
    runner.move_vertical_to(
        retreat_link6,
        f"배치 완료 후 TCP {np.round(retreat_tcp_target, 5)}로 수직 후퇴",
    )
    step_world(world, WAYPOINT_SETTLE_STEPS)
...
def main() -> None:
    ...
    # 중요: USD의 낮은 drive stiffness 상태로 physics를 한 프레임이라도 진행하면
    # 팔이 중력으로 무너진다. world.play()/world.step()보다 먼저 position controller를
    # 활성화하고 충분한 gain/effort를 적용해 reset 자세를 유지한다.
    configure_articulation_controller(robot)
