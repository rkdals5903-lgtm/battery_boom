...
def validate_and_release_cover(stage: Usd.Stage) -> None:
    ...
    # 조인트가 풀린 순간 덮개가 흡착 전에 낙하하지 않도록 조립체 중력을 끈다.
    # 흡착 후에는 Surface Gripper가 로봇을 따라 강체 전체를 운반한다.
    for body_path in [CASECOVER, *NASA_PATHS]:
        body = stage.GetPrimAtPath(body_path)
        PhysxSchema.PhysxRigidBodyAPI.Apply(body).CreateDisableGravityAttr().Set(True)
    print("[ASSEMBLY] casecover_to_casebase 해제, nasa_1~4 고정 유지, 조립체 중력 OFF")
