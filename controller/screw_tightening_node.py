...
# 조임 완료 후에는 새 케이스 이송 경로를 비워 두기 위해 모든 관절을
# 0 rad 대기 자세로 복귀한다. 조임 전 정렬(HOME_ALIGN)은 기존 자세를
# 유지해야 하므로 RETURN_HOME에서만 이 값을 사용한다.
POST_TIGHTENING_HOME_JOINT_POSITIONS = np.zeros(6, dtype=float)
...
FAR_SCREW_WAYPOINT_TIMEOUT_STEPS = 960
FAR_SCREW_APPROACH_TIMEOUT_STEPS = 960
...
# BatteryFactoryTask.enable_new_case_cover_physics()가 운반 중 나사가 cover를
# 따라오도록 nasa_X_to_casecover FixedJoint를 임시로 켠다. 네 나사 조임이
# 끝나면 이 노드가 모든 조립 joint를 끄고 완성 팩을 kinematic으로 동결한다.
SCREW_ROTATE_INCREMENT = -0.3 * SCREW_SEQUENCE_SPEED_SCALE

# joint_1은 HOME_JOINT_POSITIONS의 0.52rad 시작 정렬로 new_case 방향을
# 제한하고, RMPFlow 하드 리밋은 원본에서 검증한 joint_2의 0~+60도만 적용한다.
# joint_1까지 0~60도로 하드 제한하면 반대편 3·4번 나사의 IK 해가 사라졌다.
# 참고: HOME의
...
class ScrewTighteningNode(Node):
    ...
    def _trigger_completed_pack(self) -> None:
        ...
        if result is None or not result.success:
            ...
            raise RuntimeError(f"완성 new_case 컨베이어 이송 실패: {detail}")
        self.get_logger().info(f"[COMPLETED PACK CONVEYOR COMPLETE] {result.message}")
    ...
    def _prepare_limited_config(
        ...
        동일한 방식 — JOINT_LIMITS_DEG에 있는 joint_2의
        ...
        missing_limits = sorted(set(JOINT_LIMITS_DEG) - limited_joints)
        if missing_limits:
            raise RuntimeError(
                f"조임 URDF에서 제한할 관절을 찾지 못했습니다: {missing_limits}"
            )
        try:
    ...
    def _freeze_completed_pack(self, case_top_path: str) -> None:
        """완성 팩을 constraint 없는 안정적인 kinematic 조립체로 고정한다.

        겹침 오차가 있는 여러 dynamic body를 FixedJoint로 묶으면 PhysX가 조립
        직후 큰 보정 impulse를 만들 수 있다. 완제품은 내부 물리 시뮬레이션이
        필요 없으므로 casebase/cover/screw를 현재 pose에서 kinematic으로
        고정하고 조립 joint를 모두 끈다. 적재 셀은 기존 non-physical visual
        proxy 상태를 명시적으로 재확인한다.
        """
        stage = omni.usd.get_context().get_stage()
        component_paths = [
            f"{case_top_path}/casebase",
            f"{case_top_path}/casecover",
            *[f"{case_top_path}/nasa_{index}" for index in range(1, 5)],
        ]
        joint_names = [
        ...
        cell_paths = (
            list(self._get_completed_cell_prim_paths())
            if self._get_completed_cell_prim_paths is not None
            else []
        )
        if len(cell_paths) != 4:
            raise RuntimeError(
                "완성 팩에는 셀 프록시가 정확히 4개여야 합니다: "
                f"actual={len(cell_paths)}, paths={cell_paths}"
            )
        ...
        try:
            ...
            for path in component_paths:
                ...
                if not prim.IsValid():
                    raise RuntimeError(f"완성 팩 구성 Prim이 없습니다: {path}")
            ...
            for name in joint_names:
                joint_path = f"{case_top_path}/AssemblyJoints/{name}"
                joint = stage.GetPrimAtPath(joint_path)
                if not joint.IsValid():
                    raise RuntimeError(f"완성 팩 조립 Joint가 없습니다: {joint_path}")
                UsdPhysics.Joint(joint).CreateJointEnabledAttr().Set(False)
                stage.OverridePrim(joint_path).SetActive(False)
                disabled_joints.append(joint_path)

            for path in cell_paths:
                cell = stage.GetPrimAtPath(path)
                if not cell.IsValid():
        ...
        self.get_logger().info(
            "[PACK FREEZE] completed battery pack is kinematic: "
            f"components={len(component_paths)}, cells={len(cell_paths)}, "
            f"disabled_joints={len(disabled_joints)}"
        )
    ...
    def _handle_run(self, request, response) -> Trigger.Response:
        ...
        try:
            ...
            if len(screw_prims) != 4:
                raise RuntimeError(
                    f"나사 조이기는 정확히 4개가 필요합니다: actual={len(screw_prims)}"
                )
            ...
            if (
                not pack_frozen
                and kinematic_attr is not None
                and kinematic_attr.IsValid()
            ):
        ...
        while self._world.is_playing():
            if phase == "HOME_ALIGN":
                ...
                if step_count > 10 and target_horizontally_aligned(RMP_HORIZONTAL_TOLERANCE):
                    ...
                    target_index == 2
                    and using_third_intermediate_waypoint
                    and step_count >= THIRD_WAYPOINT_TIMEOUT_STEPS
                ):
                    # 2번 -> 3번의 긴 횡이동용 중간점만 제한시간 뒤
                    # 실제 3번 상공 목표로 넘긴다. 실제 3번 상공에서는 아래의
                    # FAR_SCREW_WAYPOINT_TIMEOUT_STEPS까지 XY 도달을 확인해야 하며,
                    # 예전처럼 120 frame 뒤 미도달 상태로 하강하지 않는다.
                    using_third_intermediate_waypoint = False
                    update_target(target_index, is_hover=True)
                    step_count = 0
                    continue
                    ...
                    waypoint_timeout = (
                        FAR_SCREW_WAYPOINT_TIMEOUT_STEPS
                        if target_index >= 2
                        else WAYPOINT_XY_TIMEOUT_STEPS
                    )
                    if step_count >= waypoint_timeout:
                        if target_index >= 2:
                            raise TimeoutError(
                                f"{target_index + 1}번 나사 상공 XY 도달 실패"
                            )
                        self.get_logger().warn(
                            f"[SCREW TIGHTEN] {target_index + 1}번 상공 XY 정렬 "
                            "제한시간 도달, 하강 진행"
                        )
                        update_target(target_index, is_hover=False)
                        phase = "APPROACH"
                ...
                step_count += 1
                approach_timeout = (
                    FAR_SCREW_APPROACH_TIMEOUT_STEPS
                    if target_index >= 2
                    else APPROACH_TIMEOUT_STEPS
                )
                if step_count > 10 and target_reached(RMP_POSITION_TOLERANCE):
                    phase = "STABILIZE"
                    step_count = 0
                elif step_count > approach_timeout:
                    if target_index >= 2:
                        raise TimeoutError(
                            f"{target_index + 1}번 나사 머리 도달 실패"
                        )
                    self.get_logger().warn(
                        f"[SCREW TIGHTEN] {target_index + 1}번 접근 제한시간 도달, "
                        "조임 진행"
                    )
                    phase = "STABILIZE"
                    step_count = 0
                ...
                if home_start_joints is None:
                    home_start_joints = self._robot.get_joint_positions()
                alpha = min(1.0, step_count / HOME_STEPS)
                interpolated_joints = (1 - alpha) * home_start_joints + alpha * POST_TIGHTENING_HOME_JOINT_POSITIONS
                self._robot.apply_action(ArticulationAction(joint_positions=interpolated_joints))
                self._world.step(render=True)
                step_count += 1
                if step_count >= HOME_STEPS:
