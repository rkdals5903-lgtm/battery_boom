...
# 사용자 정의 M0609 컨트롤러와 RMPFlow 설정 파일은 프로젝트 구성에 따라
# 실행 스크립트와 같은 폴더 또는 하위 rmpflow 폴더에 둘 수 있다.
# 실제 M0609 프로젝트 경로.
# 실행 스크립트를 Downloads에서 실행해도 프로젝트의 컨트롤러와 설정을 찾도록 한다.
_M0609_PROJECT_DIR = Path("/home/rokey/cobot3_ws/isaacpjt/M0609")

_SUPPORT_DIRS = [
    _THIS_DIR,
    _THIS_DIR / "rmpflow",
    _M0609_PROJECT_DIR,
    _M0609_PROJECT_DIR / "rmpflow",
]


def _find_support_file(filename: str) -> Path:
    """지원 파일을 현재 폴더와 rmpflow 폴더에서 순서대로 찾는다."""
    ...
    checked = "\n".join(
        f"  - {directory / filename}" for directory in _SUPPORT_DIRS
    )
    raise FileNotFoundError(
        f"필수 파일 '{filename}'을 찾지 못했습니다. 다음 위치를 확인하세요:\n"
        f"{checked}"
    )


# 컨트롤러가 들어 있는 디렉토리를 Python 모듈 검색 경로에 등록한다.
_CONTROLLER_PATH = _find_support_file("m0609_pick_place_controller.py")
_CONTROLLER_DIR = str(_CONTROLLER_PATH.parent)
...
# ╚══════════════════════════════════════════════════════════════╝
USD_PATH        = str(_find_support_file("m0609_camera.usd"))
...
M0609_URDF_PATH           = str(_find_support_file("m0609_isaac_sim.urdf"))
M0609_DESCRIPTION_PATH    = str(_find_support_file("m0609_description.yaml"))
M0609_RMPFLOW_CONFIG_PATH = str(_find_support_file("m0609_rmpflow_common.yaml"))
...
def validate_required_files():
    ...
    if missing_files:
        missing_names = "\n".join(f"  - {Path(path).name}" for path in missing_files)
        raise FileNotFoundError(
            "다음 필수 파일을 4_pick_place.py와 같은 디렉토리에 넣어주세요:\n"
            f"{missing_names}"
        )
