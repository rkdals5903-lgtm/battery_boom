...
# STEP 조립품을 USD로 변환하기 위한 CAD Converter.
# 이미 변환된 battery_assembly_converted.usd가 있으면 변환하지 않는다.
try:
    enable_extension("omni.kit.converter.hoops_core")
except Exception as exc:
    print(f"[경고] CAD Converter extension 활성화 실패: {exc}")
...
def _resolve_battery_step() -> Path:
    """배터리 STEP 파일을 실행 위치와 프로젝트 폴더에서 찾는다."""
    candidates = [
        _THIS_DIR / "battery_assembly.step",
        Path.home() / "Downloads" / "battery_assembly.step",
        M0609_PROJECT_DIR / "battery_assembly.step",
    ]
    ...
    checked = "\n".join(f"  - {path}" for path in candidates)
    raise FileNotFoundError(
        "배터리 STEP 파일 'battery_assembly.step'을 찾지 못했습니다.\n"
        "다음 위치 중 한 곳에 파일을 두세요:\n"
        f"{checked}"
    )


BATTERY_STEP_PATH = _resolve_battery_step()
BATTERY_USD_PATH = BATTERY_STEP_PATH.with_name(
    "battery_assembly_converted.usd"
)
...
# ── 배터리 물리 파라미터 ──────────────────────────────────────
BATTERY_PRIM_PATH = "/World/target_battery"
BATTERY_VISUAL_PATH = f"{BATTERY_PRIM_PATH}/Visual"
BATTERY_COLLISION_PATH = f"{BATTERY_PRIM_PATH}/CollisionProxy"

# 조립품 전체를 하나의 강체로 취급한다.
BATTERY_MASS = 6.0

# Ground Plane과 초기 접촉이 겹치지 않도록 아주 조금 띄운다.
BATTERY_GROUND_CLEARANCE = 0.002

# CAD 형상보다 충돌 박스를 약간 작게 만들어 바닥/그리퍼 접촉을 안정화한다.
BATTERY_COLLISION_SHRINK = 0.96

# STEP에서 변환된 배터리 외형을 X축 기준으로 회전한다.
# 양의 값은 오른손 좌표계 기준 +X축 회전이다.
BATTERY_ROTATION_X_DEG = 90.0
...
# Z 좌표는 CAD 모델의 실제 높이를 측정한 뒤 자동 계산한다.
BATTERY_INIT_XY = np.array([0.30, 0.40])
GOAL_XY         = np.array([0.55, -0.35])
...
EVENTS_DT = [
    0.008,   # 0. 접근 이동        (~125 스텝) 배터리 위 h1 높이로
    0.005,   # 1. 하강            (~200 스텝) 부착점을 배터리까지 내린다
...
def convert_battery_step_to_usd():
    """STEP 조립품을 USD로 변환한다. 최신 USD가 있으면 재사용한다."""
    source = Path(BATTERY_STEP_PATH)
    output = Path(BATTERY_USD_PATH)

    if output.is_file() and output.stat().st_mtime >= source.stat().st_mtime:
        print(f"  [OK] 기존 배터리 USD 재사용: {output}")
        return

    print("\n" + "=" * 60)
    print("[0.CAD] 배터리 STEP → USD 변환")
    print("=" * 60)
    print(f"  입력: {source}")
    print(f"  출력: {output}")
    ...
    try:
        ...
        raise RuntimeError(
            "CAD Converter 모듈을 불러오지 못했습니다.\n"
            "Isaac Sim Extension Manager에서 "
            "'CAD Converter' 또는 'omni.kit.converter.hoops_core'를 "
            "활성화한 뒤 다시 실행하세요.\n"
            f"원본 오류: {exc}"
        ) from exc

    # HOOPS의 file_format_args는 반드시 Dict[str, str]이어야 한다.
    # 정수·실수·bool을 그대로 넘기면 Parameters.parseArgs()에서 TypeError가 발생한다.
    options = {
        "compositionStyle": "0",
        "instancingStyle": "0",
        "tessLOD": "2",
    ...
    async def _run_converter(file_format_args):
        converter = get_instance()
        if converter is None:
            raise RuntimeError(
                "HOOPS Converter instance를 가져오지 못했습니다. "
                "omni.kit.converter.hoops_core extension 활성화 상태를 확인하세요."
            )
    ...
    def _wait_for_future(future):
        # Kit의 비동기 작업이 진행되도록 앱 이벤트 루프를 갱신한다.
        while not future.done():
            simulation_app.update()
        return future.result()

    try:
        print(f"  변환 옵션: {options}")
        result = _wait_for_future(
            asyncio.ensure_future(_run_converter(options))
        )
    except Exception as first_exc:
        # 빌드별로 일부 옵션 이름을 지원하지 않는 경우를 대비한 최소 옵션 재시도.
        # 출력 파일이 일부 생성되었을 수 있으므로 삭제한 뒤 다시 실행한다.
        print(
            "  [경고] 지정 옵션 변환 실패. 기본 옵션으로 재시도합니다. "
            f"원인: {first_exc}"
        )
        ...
        try:
            result = _wait_for_future(
                asyncio.ensure_future(_run_converter({}))
            )
        except Exception as second_exc:
            raise RuntimeError(
                "배터리 STEP을 USD로 변환하지 못했습니다.\n"
                f"문자열 옵션 변환 오류: {first_exc}\n"
                f"기본 옵션 재시도 오류: {second_exc}"
            ) from second_exc

    # 일부 converter 버전은 결과값 없이 완료되므로 파일 생성 여부로 최종 판정한다.
    for _ in range(10):
        if output.is_file():
            break
        simulation_app.update()
    ...
    if not output.is_file():
        raise RuntimeError(
            "CAD Converter 작업은 끝났지만 USD 파일이 생성되지 않았습니다.\n"
            f"예상 출력 위치: {output}\n"
            f"Converter 반환값: {result}"
        )

    print(f"  [OK] 배터리 USD 변환 완료: {output}")
    print(f"  [OK] Converter 반환값: {result}")
...
def compute_world_bbox(stage, prim_path: str):
    ...
    if not np.all(np.isfinite(bbox_min)) or not np.all(np.isfinite(bbox_max)):
        raise RuntimeError(
            f"배터리 Bounding Box 계산 결과가 유효하지 않습니다: "
    ...
    if np.any(dimensions <= 0.0):
        raise RuntimeError(
            f"배터리 크기가 올바르지 않습니다: {dimensions}"
        )
...
class M0609Task(BaseTask):
    ...
    def _create_scene(self, scene):
        print("\n" + "=" * 60)
        print("[5.SCENE] 배터리 작업 환경 구성")
        print("=" * 60)

        stage = omni.usd.get_context().get_stage()

        # ① 하나의 강체 루트 생성
        battery_root = UsdGeom.Xform.Define(
            stage, BATTERY_PRIM_PATH
        ).GetPrim()

        # ② 변환된 CAD USD를 Visual 하위 Prim으로 참조
        battery_visual = UsdGeom.Xform.Define(
            stage, BATTERY_VISUAL_PATH
        ).GetPrim()
        battery_visual.GetReferences().AddReference(
            str(BATTERY_USD_PATH)
        )

        # STEP 좌표계에서 들어온 배터리 외형을 X축 기준 +90° 회전한다.
        # 회전을 먼저 적용한 후 Bounding Box를 계산해야 회전된 실제 높이와
        # 충돌 박스 크기가 올바르게 계산된다.
        visual_xform = UsdGeom.XformCommonAPI(battery_visual)
        visual_xform.SetRotate(
            Gf.Vec3f(BATTERY_ROTATION_X_DEG, 0.0, 0.0),
            UsdGeom.XformCommonAPI.RotationOrderXYZ,
        )

        # Reference/Payload 로딩과 회전된 Bounding Box 갱신 대기
        for _ in range(20):
            simulation_app.update()

        # ③ CAD의 원점과 관계없이 회전된 형상의 중심을 강체 원점에 맞춘다.
        bbox_min, bbox_max, dimensions = compute_world_bbox(
            stage, BATTERY_VISUAL_PATH
        )
        visual_center = (bbox_min + bbox_max) / 2.0

        visual_xform.SetTranslate(
            Gf.Vec3d(
                float(-visual_center[0]),
                float(-visual_center[1]),
                float(-visual_center[2]),
            )
        )
        ...
        # CAD 외형을 중심 정렬한 뒤 크기를 다시 측정한다.
        _, _, dimensions = compute_world_bbox(
            stage, BATTERY_VISUAL_PATH
        )
        self._battery_dimensions = dimensions

        # 지나치게 크거나 작은 단위 오류를 조기에 확인한다.
        if np.max(dimensions) > 2.0 or np.max(dimensions) < 0.005:
            raise RuntimeError(
                "배터리 CAD 크기가 비정상적으로 보입니다. "
                f"측정 크기(m)={dimensions}. "
                "STEP 단위 또는 CAD Converter의 metersPerUnit 설정을 확인하세요."
            )

        # ④ 복잡한 조립품 메시 대신 숨겨진 박스 충돌체를 사용한다.
        # Surface Gripper 테스트에서는 이 방식이 가장 안정적이다.
        collision_dims = np.maximum(
            dimensions * BATTERY_COLLISION_SHRINK,
            np.array([0.002, 0.002, 0.002]),
        )

        collision_cube = UsdGeom.Cube.Define(
            stage, BATTERY_COLLISION_PATH
        )
        collision_cube.CreateSizeAttr(1.0)
        UsdGeom.XformCommonAPI(
            collision_cube.GetPrim()
        ).SetScale(
            Gf.Vec3f(
                float(collision_dims[0]),
                float(collision_dims[1]),
        ...
        # ⑤ 조립품 전체를 하나의 동적 Rigid Body로 설정한다.
        UsdPhysics.RigidBodyAPI.Apply(battery_root)
        mass_api = UsdPhysics.MassAPI.Apply(battery_root)
        mass_api.CreateMassAttr().Set(BATTERY_MASS)

        battery_height = float(dimensions[2])
        self._battery_init_position = np.array(
            [
                BATTERY_INIT_XY[0],
                BATTERY_INIT_XY[1],
                battery_height / 2.0 + BATTERY_GROUND_CLEARANCE,
            ],
            dtype=float,
        )
        ...
        # Scene에 등록해 reset/get_world_pose API를 사용한다.
        self._battery = scene.add(
            SingleRigidPrim(
                prim_path=BATTERY_PRIM_PATH,
                name="target_battery",
                position=self._battery_init_position,
        ...
        print(
            "  [OK] battery dimensions(m) = "
            f"{np.round(dimensions, 5)}"
        )
        print(
            f"  [OK] battery visual rotation = "
            f"X {BATTERY_ROTATION_X_DEG:.1f} deg"
        )
        print(f"  [OK] battery mass = {BATTERY_MASS:.3f} kg")
        print(f"  [OK] battery start = {self._battery_init_position}")
        print(f"  [OK] collision proxy = {np.round(collision_dims, 5)}")

        # Goal marker는 배터리 중심이 아닌 바닥 위치를 표시한다.
        scene.add(
            VisualCuboid(
                prim_path="/World/goal_marker",
                name="goal_marker",
                position=np.array(
                    [GOAL_XY[0], GOAL_XY[1], 0.0005]
                ),
                scale=np.array(
                    [
                        max(float(dimensions[0]) * 1.2, 0.06),
                        max(float(dimensions[1]) * 1.2, 0.06),
                        0.001,
                    ]
                ),
        ...
        print(f"  [OK] goal = {self._goal_position}")
    ...
    def get_observations(self):
        ...
        return {
            ...
            self._battery.name: {
                # 기존 Controller가 cube 중심 좌표를 사용하던 의미를 유지한다.
    ...
    def pre_step(self, control_index, simulation_time):
        ...
        # 조립품은 재질 계층이 복잡하므로 색 변경 대신 XY 도달로 성공 판정한다.
        xy_error = np.linalg.norm(
            self._goal_position[:2] - battery_pos[:2]
        )
        if not self._task_achieved and xy_error < 0.035:
            self._task_achieved = True
            print(
                f"  [성공 판정] 배터리 목표 XY 도달: "
                f"error={xy_error:.4f} m"
            )

    def post_reset(self):
        # 리셋 시 이전 Surface Gripper 부착 조인트를 해제한다.
...
def main():
    ...
    convert_battery_step_to_usd()

    # ── C-1. World + Task ─────────────────────────────────────
    my_world = World(stage_units_in_meters=1.0)

    # Standalone 환경에서도 Ground Plane을 Python API로 추가할 수 있다.
    # 배터리 높이는 CAD Bounding Box로 측정해 바닥 위에 자동 배치한다.
    my_world.scene.add_default_ground_plane(z_position=0.0)
    print("  [OK] Ground Plane 추가: z=0.0 m")
    ...
    print(f"  배터리 크기(m) = {task._battery_dimensions}")
    ...
    while simulation_app.is_running():
        ...
        if is_playing and not task_done:
            ...
            # (2) Controller에 배터리 중심 좌표와 목표 좌표 전달
            ...
            # 뜻일 뿐이다. 배터리를 떨어뜨렸거나 못 잡아도 True가 될 수 있다.
            # 실제 성공은 M0609Task.pre_step()의 배터리 XY 위치로 판정한다.
