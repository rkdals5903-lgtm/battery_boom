...
from battery_cover_open_node import BatteryCoverOpenNode
from grip_cell_node import GripCellNode
from BatteryVoltageServer import BatteryVoltageServer
...
# ------------------------------------------------------------
# 6-1b. 나사 분해 이후 casecover 분리/셀 공정 파라미터
# ------------------------------------------------------------
# ScrewDisassemblyNode는 기존대로 /start_battery_cover_drop을 호출한다.
# 이제 그 서비스는 배터리 전체를 버리는 대신 casecover만 분리한다.
# 아래 위치는 battery_open_sasumi.py에서 사용했던 안전한 cover 이동 영역을
# 통합 초기값으로 사용한 것이다. 실제 Stage에서 최종 튜닝 가능하다.
COVER_OPEN_PLACE_POSITION = np.array([1.28, 6.00, 0.90], dtype=float)
COVER_OPEN_DROP_HEIGHT = 0.10

# 정상 cell을 채울 새 케이스. grip_cell_node.py는 casebase BBox를 기준으로
# source battery의 slot geometry를 평행 이동해 적재 위치를 계산한다.
...
class BatteryFactoryTask(BaseTask):
    ...
    def _create_scene(self, scene) -> None:
        ...
        for battery_path in battery_paths:
            ...
            # 분리형 배터리 모델은 casebase/casecover/cell/nasa가 각각 독립
            # RigidBody이다. 이런 모델의 최상위 good_battery에도 RigidBodyAPI를
            # 덧씌우면 nested rigid body가 되어 joint/충돌/셀 분리가 깨진다.
            child_rigid_bodies = [
                str(prim.GetPath())
                for prim in Usd.PrimRange(battery_prim)
                if prim.GetPath() != battery_prim.GetPath()
                and prim.HasAPI(UsdPhysics.RigidBodyAPI)
            ]
            separable_battery = bool(child_rigid_bodies)

            if separable_battery:
                # USD에 작성된 child RigidBody/Collider/FixedJoint 설정을 존중한다.
                # main.py가 collider approximation이나 mass를 다시 덮어쓰지 않는다.
                self._battery_prims[battery_path] = None
                _, _, dimensions = compute_world_bbox(stage, battery_path)
                self._battery_dimensions_by_path[battery_path] = dimensions
                print(
                    f"  [OK] 분리형 배터리 등록: {battery_path}, "
                    f"child rigid bodies={len(child_rigid_bodies)}, "
                    f"dimensions(m)={np.round(dimensions, 5)}"
                )
                continue

            # 구형 단일 rigid-body 배터리는 기존 로직을 유지한다.
            ...
            self._battery_dimensions_by_path[battery_path] = dimensions
            print(
                f"  [OK] 단일 rigid 배터리 등록: {battery_path}, collider={collider_count}개, "
                f"dimensions(m)={np.round(dimensions, 5)}"
            )
    ...
    def get_battery_top_center_position(self, battery_path: str) -> np.ndarray:
        """배터리 전체 BBox의 윗면 중심을 반환한다.

        분리형 배터리는 최상위 root를 SingleRigidPrim으로 등록하지 않으므로
        pivot 기반 계산 대신 BBox만 사용한다. 단일/분리형 모두 같은 방식으로 동작한다.
        """
        stage = omni.usd.get_context().get_stage()
    ...
    def get_last_placed_battery_path(self) -> str:
        """현재 작업대에서 분해 중인 배터리 root path."""
    ...
    def get_last_placed_battery_position(self) -> np.ndarray:
        """마지막 작업대 배터리 전체 윗면 중심. 기존 기능 호환용."""
        return self.get_battery_top_center_position(self.get_last_placed_battery_path())

    def get_last_placed_battery_pick_yaw_deg(self) -> float:
        return self.get_battery_pick_yaw_deg(self.get_last_placed_battery_path())

    def get_battery_cover_prim_path(self) -> str:
        base = self.get_last_placed_battery_path()
        cover_path = find_prim_path_by_name(base, "casecover")
        if cover_path is None:
            raise RuntimeError(f"{base} 아래에서 casecover Prim을 찾지 못했습니다.")
        return cover_path

    def get_battery_cover_joint_path(self) -> str:
        base = self.get_last_placed_battery_path()
        joint_path = find_prim_path_by_name(base, "casecover_to_casebase")
        if joint_path is None:
            raise RuntimeError(f"{base} 아래에서 casecover_to_casebase FixedJoint를 찾지 못했습니다.")
        return joint_path

    def get_battery_cover_position(self) -> np.ndarray:
        """VG10이 흡착할 casecover의 실제 BBox 윗면 중심."""
        stage = omni.usd.get_context().get_stage()
        cover_path = self.get_battery_cover_prim_path()
    ...
    def get_battery_cover_pick_yaw_deg(self) -> float:
        stage = omni.usd.get_context().get_stage()
        bbox_min, bbox_max, _ = compute_world_bbox(stage, self.get_battery_cover_prim_path())
        return 90.0 if (bbox_max[0] - bbox_min[0]) > (bbox_max[1] - bbox_min[1]) else 0.0

    def clear_last_placed_battery(self) -> None:
        # grip-cell 부분 적재 확장에서는 이 상태를 다음 배터리 요청 때도 사용할 수
        # 있으므로 자동으로 호출하지 않는다. 필요한 공정 완료 지점에서 명시 호출한다.
        self._last_placed_battery_path = None

    def get_battery_screw_prim_paths(self) -> Optional[list]:
        """현재 작업대 배터리의 나사 4개 Prim 경로를 반환한다.

        새 분리형 배터리(nasa_1~4)를 우선 사용하고, 기존 CAD 배터리의
        tn__Part19* 구조도 fallback으로 유지한다.
        """
        if self._last_placed_battery_path is None:
            return None
        stage = omni.usd.get_context().get_stage()
        ...
        nasa_paths = [find_prim_path_by_name(base, f"nasa_{i}") for i in range(1, 5)]
        if all(path is not None and stage.GetPrimAtPath(path).IsValid() for path in nasa_paths):
            return nasa_paths
        ...
        if all(stage.GetPrimAtPath(path).IsValid() for path in legacy):
            return legacy

        print(f"[WARN] {base} 아래에서 nasa_1~4 또는 legacy screw 4개를 찾지 못했습니다.")
        return None
...
def main() -> None:
    ...
    # --------------------------------------------------------
    # Cell voltage node + GripCellNode
    # --------------------------------------------------------
    # 같은 main 프로세스 안에서 voltage_node.sample_voltage()를 직접 주입한다.
    # ROS service를 자기 자신에게 동기 호출하지 않으므로 single spin loop deadlock이 없다.
    voltage_node = BatteryVoltageServer()

    grip_cell_node = GripCellNode(
        world=my_world,
        robot=robot,
        get_battery_root=task.get_last_placed_battery_path,
        voltage_sampler=voltage_node.sample_voltage,
        voltage_min=voltage_node.MIN_VOLTAGE,
        voltage_max=voltage_node.MAX_VOLTAGE,
        # BatteryVoltageServer.py의 "생성 분포 중심값"을 판정 기준으로 사용.
        # 현재 11.0 V: 아래=False, 이상=True.
        voltage_threshold=voltage_node.MEAN_VOLTAGE,
        urdf_path=M0609_URDF_PATH,
        robot_description_path=M0609_DESCRIPTION_PATH,
        rmpflow_config_path=M0609_RMPFLOW_CONFIG_PATH,
        robot_root_path=M0609_RG2_PRIM_PATH,
        tool_length_m=float(RG2_EE_OFFSET[2]),
        new_case_root=NEW_CASE_ROOT_PRIM_PATH,
        end_effector_frame_name=M0609_EE_LINK_NAME,
        # grip_cell_final.py가 mock_suction_close.py에 보내던 endpoint와 동일.
        cover_close_service_name="/suction_cover_close",
    )

    # --------------------------------------------------------
    # 나사 분해 -> actual casecover open -> grip cell
    # --------------------------------------------------------
    # ScrewDisassemblyNode가 이미 호출하는 /start_battery_cover_drop 이름은
    # 그대로 유지한다. 단, 동작은 이제 '전체 배터리 폐기'가 아니라 casecover만
    # 흡착/분리한다. 완료 콜백이 GripCellNode.request_start()를 호출하므로
    # mock_server.py(/suction_cover_opened)는 필요 없다.
    battery_cover_open_node = BatteryCoverOpenNode(
        world=my_world,
        robot=vg10_robot,
        get_picking_position=task.get_battery_cover_position,
        get_cover_joint_path=task.get_battery_cover_joint_path,
        placing_position=COVER_OPEN_PLACE_POSITION,
        end_effector_offset=VG10_SURFACE_LOCAL_OFFSET,
        on_cover_opened=grip_cell_node.request_start,
        get_pick_yaw_deg=task.get_battery_cover_pick_yaw_deg,
        service_name="/start_battery_cover_drop",
        controller_kwargs=dict(
            name="m0609_vg10_cover_open_controller",
            gripper=vg10_robot.gripper,
            robot_articulation=vg10_robot,
            urdf_path=M0609_URDF_PATH,
            robot_description_path=M0609_DESCRIPTION_PATH,
            rmpflow_config_path=M0609_RMPFLOW_CONFIG_PATH,
            end_effector_frame_name=M0609_EE_LINK_NAME,
            place_drop_height=COVER_OPEN_DROP_HEIGHT,
        ),
    )
    ...
    print("Cover open : /start_battery_cover_drop service 대기 중 (기존 screw 연동 유지)")
    print("Grip cell  : cover-open 완료 콜백 또는 /start_grip_cell_process")
    print("Voltage    : /check_voltage + 내부 sample_voltage()")
    print("Cover close: 정상 cell 4/4이면 /suction_cover_close 요청 전송")
    print("=" * 60 + "\n")
    ...
    while simulation_app.is_running():
        ...
        rclpy.spin_once(voltage_node, timeout_sec=0.0)
        rclpy.spin_once(grip_cell_node, timeout_sec=0.0)
        rclpy.spin_once(battery_cover_open_node, timeout_sec=0.0)

        # cover-open callback은 GripCellNode에 pending start만 세팅한다.
        # 실제 긴 셀 공정은 여기서 실행해 ROS callback 중첩/deadlock을 피한다.
        ...
        if is_playing and not was_playing:
            ...
            battery_cover_open_node.reset_controller()
    ...
    battery_cover_open_node.destroy_node()
