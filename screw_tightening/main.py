...
class BatteryFactoryTask(BaseTask):
    ...
    def get_last_placed_battery_cover_position(self) -> np.ndarray:
        """작업대에 마지막으로 놓인 배터리의 casecover 위치(xform pivot).
        BatteryCoverDropNode 전용 — casecover만 집어서 버릴 때 쓴다."""
        if self._last_placed_battery_path is None:
            raise RuntimeError("작업대에 아직 배터리가 놓인 적이 없습니다.")
        position = self.get_battery_cover_center_position(self._last_placed_battery_path)
    ...
    def get_new_case_screw_prim_paths(self) -> Optional[list]:
        """new_battery_01(NEW_CASE_ROOT_PRIM_PATH)의 나사 4개 prim 경로를
        반환한다. ScrewTighteningNode 전용 — get_battery_screw_prim_paths()와
        동일한 물리 순서(1, 2, 4, 3)를 쓴다. casecover가 아직 안 닫혀서(=
        SuctionCoverCloseNode가 실행되기 전) nasa가 비활성 상태면 None을
        반환한다 — enable_new_case_cover_rigid_body()가 GRIP 시점에 활성화
        해 주므로, 조이기는 그 이후(뚜껑 닫기 완료)에만 의미가 있다.
        """
        base = NEW_CASE_ROOT_PRIM_PATH
        stage = omni.usd.get_context().get_stage()
        nasa_1 = stage.GetPrimAtPath(f"{base}/nasa_1")
        if not nasa_1.IsValid() or not nasa_1.IsActive():
            return None
        return [
            f"{base}/nasa_1",
            f"{base}/nasa_2",
...
def main() -> None:
    ...
    # --------------------------------------------------------
    # new_battery_01 뚜껑 닫기가 끝나면(SuctionCoverCloseNode의
    # /start_screw_tightening 신호) 같은 스크류 팔(m0609_screw_robot)과 드라이버
    # 공구(screw_tool)를 재사용해 나사를 조인다. screw_disassembly_node.py와
    # 구조가 동일하고 회전 방향만 반대다 — 서비스 호출로 실행 순서가
    # 직렬화돼 있어(뚜껑 닫기 완료 후에만 호출됨) 나사 분해와 동시에 이 팔을
    # 움직이는 일은 없다.
    # --------------------------------------------------------
    screw_tightening_node = ScrewTighteningNode(
        ...
        robot=m0609_screw_robot,
        screw_tool=screw_tool,
        get_new_case_screw_prim_paths=task.get_new_case_screw_prim_paths,
        controller_kwargs=dict(
    ...
    # OUTFEED_ORDER/OUTFEED_PALLET_DESTINATION_POSITION이 전부 placeholder라
    # 지금은 서비스를 호출해도 "옮길 대상 없음"으로 안전하게 끝난다.
    # --------------------------------------------------------
