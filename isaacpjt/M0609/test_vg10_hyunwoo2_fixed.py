...
TABLE_PRIM_PATH   = "/World/work_table_01"
...
# 목표 위치: 실측/조정된 틀 좌표(2026-08-05).
PLACE_TARGET_POSITION = np.array([1.76929, 6.49723, 1.0123])
...
J1_HOME_DEG = 90.0
J1_PLACE_DEG = 180.0  # ROTATE_J1 단계에서 J1이 도달할 목표 각도
...
class M0609PrebuiltEnvTask(BaseTask):
    ...
    def _compute_place_target(self):
        """틀(ws_fix_real_final) 중심 실측 좌표를 목표 위치로 사용한다."""
        print("\n" + "=" * 60)
        print("[6.PLACE TARGET] 틀 중심 좌표 (실측값 고정)")
        print("=" * 60)
        self._goal_position = np.array(PLACE_TARGET_POSITION, dtype=float)
        print(f"  [OK] 목표 위치(틀 중심, 고정) = {self._goal_position}")
